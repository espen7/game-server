...
public class OrionServices {
    ...
    public static void sendToService(ActorSystem system, String serviceName, Object message, ActorRef sender) {
        ...
        // 对于 Put/Send，我们需要路径。如果我们使�?Subscribe/Publish，它是广播�?
        // 要发送给任何单个实例，Send 更好，但要求 actor �?Put�?
        // Put 中的路径通常�?self.path().withoutAddress()�?
        
        // 此框架的简化方法：使用 Publish 到主题，但这会广播到组�?
        // 更好：使用带路径�?Send。但如果是动态的，我们不知道路径�?
        // 让我们使用一致的命名约定或仅使用 Group Router�?
        
        // 实际上，让我们使�?DistributedPubSub �?SendToAll 或仅 Send（如果我们知道路径）�?
        // 但如果是动态的，我们不知道路径�?
        
        // 替代方案：用于单例的 ClusterSingleton�?
        // 对于示例，让我们使用带有主题�?DistributedPubSub 并发送给一个�?
        // DistributedPubSub 不容易支持“发送给主题的一个”而不需要组�?
        
        // 让我们使用带有众所周知路径约定�?Send？不，那很脆弱�?
        
        // 让我们使用基于“角色”的路由或仅用于示例的简�?ActorSelection�?
